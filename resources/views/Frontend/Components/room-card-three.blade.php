                ...
                <li class="py-2 px-3 rounded bg-light d-flex align-items-center border mb-2">
                    <div class="text-dark">
                        <p class="mb-0 fw-bold small">
                            {{ $slot['name'] ?? 'Time Slot' }}
                            @if(isset($slot['start_time']) && !($slot['is_overnight'] ?? false))
                            <span class="small">({{ \Carbon\Carbon::parse($slot['start_time'])->format('g:i A') }} - {{ \Carbon\Carbon::parse($slot['end_time'])->format('g:i A') }})</span>
                            @endif

                            @if(isset($slot['nights']) && $slot['nights'] > 0)
                            <span class="small">({{ $slot['nights'] }} {{ Str::plural('night', $slot['nights']) }})</span>
                            @endif
                        </p>
                    </div>
                    <span class="ms-auto small mb-0 text-dark fw-bold">
                        @if(array_key_exists('price', $slot) && !is_null($slot['price']))
                            ${{ number_format($slot['price']) }}
                        @elseif(array_key_exists('price_per_night', $slot) && !is_null($slot['price_per_night']))
                            ${{ number_format($slot['price_per_night']) }} / night
                        @elseif(array_key_exists('total_price', $slot) && !is_null($slot['total_price']))
